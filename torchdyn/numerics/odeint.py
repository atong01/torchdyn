...
def odeint(f:Callable, x:Tensor, t_span:Union[List, Tensor], solver:Union[str, nn.Module], atol:float=1e-3, rtol:float=1e-3,
		   ...
		   save_at:Union[Iterable, Tensor]=(), seminorm:Tuple[bool, Union[int, None]]=(False, None), *args, **kwargs) -> Tuple[Tensor, Tensor]:
	...
	Args:
		...
		args (List): Arbitrary parameters used in step.
		kwargs (Dict): Arbitrary keyword parameters used in step.
	...
	# odeint routine with a single t_span for all samples
		...
		if stepping_class == 'fixed':
			...
			return _fixed_odeint(f_, x, t_span, solver, save_at=save_at, *args, **kwargs)
			...
			return _adaptive_odeint(f_, k1, x, dt, t_span, solver, atol, rtol, interpolator, return_all_eval, seminorm, *args, **kwargs)
...
def _adaptive_odeint(f, k1, x, dt, t_span, solver, atol=1e-4, rtol=1e-4, interpolator=None, return_all_eval=False, seminorm=(False, None), *args, **kwargs):
	...
	Args:
		...
		return_all_eval (bool, optional): Defaults to False.
		args (List): Arbitrary extra argments given to step.
		kwargs (Dict): Arbitrary keyword arguments given to step.
	...
	while t < T:
		...
		f_new, x_new, x_err, stages = solver.step(f, x, t, dt, k1=k1, *args, **kwargs)
		################# compute error #############################
		if seminorm[0] == True:
			state_dim = seminorm[1]
			error = x_err[:state_dim]
...
def _fixed_odeint(f, x, t_span, solver, save_at=(), *args, **kwargs):
	...
	while steps <= len(t_span) - 1:
		_, x, _ = solver.step(f, x, t, dt, k1=None, *args, **kwargs)
		t = t + dt
